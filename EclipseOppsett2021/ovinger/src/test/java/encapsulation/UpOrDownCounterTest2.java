...
public class UpOrDownCounterTest2{
	...
	private void testCount(UpOrDownCounter counter, int end, int delta) {
		...
		while (delta > 0 ? counter.getCounter() < end : counter.getCounter() > end) {
			...
			Assertions.assertEquals(expected, counter.getCounter(),"When counting from " + i + " the result should be " + expected + ". ");
		}
		Assertions.assertFalse( result,"When reaching the end value, the count method should return false. ");
		Assertions.assertEquals(end, counter.getCounter(), "After reaching the end value, the counter should not change. ");
		Assertions.assertFalse(counter.count(),"After reaching the end value, the count() method should return false. ");
		Assertions.assertEquals(end, counter.getCounter(), "After reaching the end value, the counter should not change. ");
	}
	
	public void testUpOrDownCounter() {
		UpOrDownCounter counter13 = new UpOrDownCounter(1, 3);
		Assertions.assertEquals(1, counter13.getCounter(), "After instantiating an UpOrDownCounter the counter should be the start value. ");
		UpOrDownCounter counter31 = new UpOrDownCounter(3, 1);
		Assertions.assertEquals(3, counter31.getCounter(), "After instantiating an UpOrDownCounter the counter should be the start value. ");
	}
